<template>
  <div class="activity-item" :class="actTypeClass">
...
<style scoped>
.activity-item {
  display: flex; gap: 1.5rem; padding: 1.5rem; border-radius: var(--radius-xl);
  background: #fff; box-shadow: var(--shadow-sm); border: 1px solid var(--border); position: relative; overflow: hidden; transition: all .4s var(--ease-out); opacity: 1;
}
.activity-item:hover { box-shadow: var(--shadow-md); transform: translateY(-2px); }
.activity-item::before { content: ''; position: absolute; left: 0; top: 0; bottom: 0; width: 6px; background: var(--border-strong); transition: width .3s var(--ease-out); }
...
.act-time { flex: 0 0 100px; padding-top: .25rem; font-family: var(--font-display); font-size: 1.25rem; font-weight: 500; color: var(--text-primary); line-height: 1.1; }
.act-content { flex: 1; min-width: 0; }
.act-header { display: flex; justify-content: space-between; align-items: flex-start; gap: 1rem; margin-bottom: .5rem; }
.act-title { font-size: 1.125rem; font-weight: 600; color: var(--text-primary); line-height: 1.4; }
.act-desc { font-size: var(--fs-body); color: var(--text-secondary); margin-bottom: .75rem; line-height: 1.6; }
.act-notes-wrap { margin: .75rem 0; padding: .75rem 1rem; background: var(--bg-raised); border-radius: var(--radius-sm); border-left: 2px solid var(--border-strong); position: relative; }
.act-notes { font-size: var(--fs-caption); color: var(--text-secondary); line-height: 1.6; font-style: italic; white-space: pre-wrap; }
.act-notes-wrap.collapsed .act-notes { display: -webkit-box; -webkit-line-clamp: 2; -webkit-box-orient: vertical; overflow: hidden; }
.notes-expand { display: inline-block; margin-top: .5rem; font-size: 11px; font-weight: 500; color: var(--accent); background: var(--accent-soft); padding: .2rem .6rem; border-radius: var(--radius-full); }
.act-meta { display: flex; flex-wrap: wrap; gap: 1rem; margin-top: 1rem; font-size: var(--fs-caption); }
.act-meta a { display: flex; align-items: center; gap: .35rem; color: var(--accent); font-weight: 500; background: var(--accent-soft); padding: .4rem .8rem; border-radius: var(--radius-full); transition: all .2s; }
.act-meta a:hover { background: var(--accent-glow); color: var(--accent-deep); }

.act-actions { display: flex; gap: .5rem; opacity: 0; transform: translateX(10px); transition: all .3s var(--ease-out); pointer-events: none; }
.activity-item:hover .act-actions { opacity: 1; transform: translateX(0); pointer-events: auto; }
.act-btn { width: 36px; height: 36px; border-radius: 50%; background: var(--bg-raised); color: var(--text-secondary); display: flex; align-items: center; justify-content: center; border: 1px solid var(--border); transition: all .2s; }
...
@media (max-width: 600px) {
  .activity-item { flex-direction: column; gap: .75rem; padding: 1.25rem; border-radius: 20px; }
  ...
  .act-actions { position: static; opacity: 1; transform: none; pointer-events: auto; gap: 8px; flex-shrink: 0; }
  .act-btn { width: 36px; height: 36px; background: var(--bg-deep); }
}
</style>
