<template>
  <div class="modal-overlay" :class="{ active: visible }" id="activityModal" @click.self="closeModal">
    <div class="modal" @click.stop>
      ...
      <div class="modal-body">
        <form class="clean-form" id="activityForm" @submit.prevent="save">
          ...
          <DayChips 
            :days="tripStore.days" 
            :selected-day="localDayIndex" 
            @update:selectedDay="(val) => localDayIndex = val" 
          />

          <div class="form-group">
            <label class="form-label" for="actTitle">活動名稱 *</label>
            <input type="text" id="actTitle" class="form-input" v-model="formData.title" placeholder="例如：首里城、居酒屋晚餐" required>
          </div>
          ...
          <div class="form-group">
            ...
            <input type="url" id="actMapUrl" class="form-input" v-model="formData.mapUrl" placeholder="https://maps.app.goo.gl/...">
          </div>
        </form>
      </div>
...
function save() {
  if (!formData.value.title.trim()) {
    showToast('請填寫活動名稱')
    const input = document.getElementById('actTitle')
